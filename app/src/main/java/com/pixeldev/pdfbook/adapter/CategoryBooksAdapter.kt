...
import com.pixeldev.pdfbook.databinding.CategoryBooksLightBinding
...
class CategoryBooksAdapter(val mContext: Context, var mData: List<Image>) :
    RecyclerView.Adapter<RecyclerView.ViewHolder?>() {
    var option = RequestOptions().centerCrop()
    var sharedPref: SharedPref? = null
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val view2: View
        val sharedPref2 = SharedPref(mContext)
        sharedPref = sharedPref2

        view2 = if (!sharedPref2.loadNightModeState()) {
            LayoutInflater.from(mContext).inflate(R.layout.category_books_light, parent, false)

        } else {
            LayoutInflater.from(mContext).inflate(R.layout.category_books_dark, parent, false)
        }
        return when (viewType) {
            ITEM_VIEW -> MainViewHolder(view2)
...
//                view2 = layoutInflater.inflate(R.layout.category_books_light, parent, false)
//            } else {
//                view2 = layoutInflater.inflate(R.layout.category_books_dark, parent, false)
//            }
//            return MainViewHolder(view2)
//        } else if (viewType != 1) {
//            return null
//        } else {
//            view = layoutInflater.inflate(R.layout.item_native_ad, parent, false)
...
//    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder!!.itemViewType == 0) {
            (holder as MainViewHolder?)!!.bindData(
                mData[position - Math.round((position / ITEM_FEED_COUNT).toFloat())]
            )
    ...
    inner class MainViewHolder(itemView: View?) : RecyclerView.ViewHolder(
        ...
        var binding: CategoryBooksLightBinding? = null

        init {
            sharedPref = SharedPref(mContext)
            if (sharedPref!!.loadNightModeState()) {
                binding = CategoryBooksLightBinding.bind((itemView)!!)
            } else {
                binding = CategoryBooksLightBinding.bind((itemView)!!)
            }
        }
        ...
        fun bindData(main: Image) {
            val categoryBooksAdapter = this@CategoryBooksAdapter
            categoryBooksAdapter.sharedPref = SharedPref(categoryBooksAdapter.mContext)
            if (sharedPref!!.loadNightModeState()) {
                ...
                if (!cursor!!.moveToNext()) {
                    ...
                    if (sharedPref!!.loadNightModeState()) {
                        binding!!.favBtn.buttonDrawable =
                            mContext.getDrawable(R.drawable.iv_favorite_vd_theme_light)
                    } else {
                        binding!!.favBtn.buttonDrawable =
                            mContext.getDrawable(R.drawable.ic_favorite_vd_theme_24px)
                    }
                    binding!!.favBtn.isChecked = false
                } else if (sharedPref!!.loadNightModeState()) {
                    binding!!.favBtn.buttonDrawable =
                        mContext.getDrawable(R.drawable.ic_fav_outline_category)
                } else {
                    binding!!.favBtn.buttonDrawable =
                        mContext.getDrawable(R.drawable.ic_baseline_faorite_border_white)
                }
            }
